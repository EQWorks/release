...
word_embedding = keras.Input(shape=(None,512),name='word_input')


convs = []
for kernel_size in [2]:
    conv_output= layers.Conv1D(kernel_size=kernel_size, filters=200, padding='causal',activation='relu', strides=1)(word_embedding)
    maxpool_output = layers.MaxPooling1D(2)(conv_output)
...
lstm_forward1 = layers.LSTM(100, activation='relu',return_sequences=True, dropout=0.1, recurrent_dropout=0.1)(con_convs)
...
lstm_backward1 = layers.LSTM(100,activation='relu', return_sequences=True, dropout=0.1, recurrent_dropout=0.1,go_backwards = True)(con_convs)
...
dense_layer1 = layers.Dense(100,activation='relu')(con_layer2)

droup_out2 = layers.Dropout(.5)(dense_layer1)

dense_layer2 = layers.Dense(100,activation='relu')(droup_out2)
...
input_sample = np.empty((total_size, *(25,512)))
    ...
    for row in data_file:
        ...
        for word in features:
            if word_i<25:
                ...
                input_sample[i,word_i] = np.array(word['layers'][3]['values'])
                word_i+=1
            else:
                break
        while word_i <25:
            #embedded_sentence.append([[0]*128])
            input_sample[i, word_i] = np.array([0]*512)
            word_i+=1
        i+=1
...
checkpoint_filepath = 'model_testb_2cnn.h5'
...
callback = keras.callbacks.EarlyStopping(monitor='val_loss', patience=2000)
